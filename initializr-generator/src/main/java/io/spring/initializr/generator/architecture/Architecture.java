...
public interface Architecture {
	...
	static Architecture forId(String id) {
		return SpringFactoriesLoader.loadFactories(ArchitectureFactory.class, Architecture.class.getClassLoader())
				.stream().map((factory) -> factory.createArchitecture(id)).filter(Objects::nonNull).findFirst()
				.orElseThrow(() -> new IllegalStateException("Unrecognized architecture id '" + id + "'"));
	}
