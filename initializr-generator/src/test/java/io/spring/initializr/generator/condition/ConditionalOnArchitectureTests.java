...
/**
 * Tests for {@link ConditionalOnArchitecture}.
 *
 * @author Duan Zhiqiang
 */
class ConditionalOnArchitectureTests {
	...
	void outcomeWithNoneArchitecture() {
		...
		assertCondition(description, (context) -> {
			...
			assertThat(context.getBean(String.class)).isEqualTo("testNone");
		});
	}

	@Test
	void outcomeWithLayeredArchitecture() {
		MutableProjectDescription description = new MutableProjectDescription();
		description.setArchitecture(new LayeredArchitecture());
		assertCondition(description, (context) -> {
			assertThat(context.getBeansOfType(String.class)).hasSize(1);
			assertThat(context.getBean(String.class)).isEqualTo("testLayered");
		});
	}
	...
	private void assertCondition(MutableProjectDescription description, Consumer<ProjectGenerationContext> context) {
		try (ProjectGenerationContext projectContext = new ProjectGenerationContext()) {
			...
			projectContext.register(ArchitectureTestConfiguration.class);
	...
	@Configuration
	static class ArchitectureTestConfiguration {

		@Bean
		@ConditionalOnArchitecture("none")
		String none() {
			return "testNone";
		}

		@Bean
		@ConditionalOnArchitecture("layered")
		String layered() {
			return "testLayered";
		}
